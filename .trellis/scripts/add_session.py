...
def count_journal_files(dev_dir: Path, active_num: int) -> str:
    ...
    for f in files:
        ...
        status = "使用中" if filename == active_file else "已归档"
        result_lines.append(f"| `{filename}` | 约 {lines} | {status} |")
...
def create_new_journal_file(
    ...
    content = f"""# {developer} 工作日志（第 {num} 部分）

> 续接自 `{FILE_JOURNAL_PREFIX}{prev_num}.md`（约 {max_lines} 行时归档）
> 开始日期：{today}
...
def update_index(
    ...
    for line in lines:
        if "@@@auto:current-status" in line:
            ...
            new_lines.append(f"- **最后活跃日期**：{today}")
            continue
        ...
        if in_session_history:
            # 将旧版中英文 4/5/6 列表头统一迁移为中文 5 列格式。
            if re.match(
                r"^\|\s*#\s*\|\s*(?:Date|日期)\s*\|\s*(?:Title|标题)\s*\|\s*(?:Commits|提交)\s*\|",
                line,
            ):
